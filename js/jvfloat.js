...
(function($) {
  ...
  $.fn.jvFloat = function () {
    ...
    return this.filter('input:not([type=submit]), textarea, select').each(function() {
      ...
      if ($(this).is('textarea')) {
        placeholder = $('<label class="placeHolder ' + ' textarea ' + required + '" for="' + forId + '"><span class="input-inner>' + placeholderText + '</span></label>').insertBefore($el);
      } else {
        placeholder = $('<label class="placeHolder ' + required + '" for="' + forId + '"><span class="input-inner">' + placeholderText + '</span></label>').insertBefore($el);
      }
